...
class RCWedge extends RCDraw, implements RCDrawInterface {
	...
	public function redraw() :Void {
		...
		this.configure();
		layer.graphics.moveTo (size.width/2, size.height/2);
		...
		if (segs > 0) {
			...
			for (i in 0...segs) {
				...
				layer.graphics.curveTo (cx, cy, bx, by);
			}
			
			// Close the wedge by drawing a line to the center
			layer.graphics.lineTo (size.width/2, size.height/2);
		}
	}
}
#end